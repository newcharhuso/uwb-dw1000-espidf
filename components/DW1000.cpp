 ...
 * ######################################################################### */
// pins
gpio_num_t DW1000Class::_ss;
gpio_num_t DW1000Class::_rst;
gpio_num_t DW1000Class::_irq;
...
void DW1000Class::reselect(gpio_num_t ss) {
	_ss = ss;
...
void DW1000Class::begin(gpio_num_t irq, gpio_num_t rst) {
	// generous initial init/wake-up-idle delay
	printf("Initializing SPI and GPIO configuration");
	vTaskDelay(5 / portTICK_PERIOD_MS);
	// start SPI
...
void DW1000Class::spiWakeup(){
        gpio_set_level(_ss,0);//digitalWrite(_ss, LOW);
        vTaskDelay(5 / portTICK_PERIOD_MS);
        gpio_set_level(_ss,1);//digitalWrite(_ss, HIGH);
...
void DW1000Class::reset() {
	if(_rst == 0xff) {
		...
		// dw1000 data sheet v2.08 §5.6.1 page 20, the RSTn pin should not be driven high but left floating.
		gpio_set_direction(_rst, GPIO_MODE_OUTPUT);//pinMode(_rst, OUTPUT);
		gpio_set_level(_rst,0);// digitalWrite(_rst, LOW);
		vTaskDelay(2/ portTICK_PERIOD_MS);// delay(2);  // dw1000 data sheet v2.08 §5.6.1 page 20: nominal 50ns, to be safe take more time
		gpio_set_direction(_rst, GPIO_MODE_INPUT);// pinMode(_rst, INPUT);
		vTaskDelay(10 / portTICK_PERIOD_MS);// delay(10); // dwm1000 data sheet v1.2 page 5: nominal 3 ms, to be safe take more time
 ...
 * #### Interrupt handling ###################################################
 * ######################################################################### */
DW1000Class* DW1000Class::_instance = nullptr;

DW1000Class* DW1000Class::instance() {
    if (_instance == nullptr) {
        _instance = new DW1000Class(); // Create a new instance if it doesn't exist
    }
    return _instance;
}
...
void DW1000Class::setData(const std::string& data) {
    ...
    uint8_t* dataBytes = new uint8_t[n];

    // Copy the data from the std::string to the byte array
    std::copy(data.begin(), data.end(), dataBytes);

    // Pass the byte array to the setData method
    setData(dataBytes, n);
...
void DW1000Class::getData(std::string& data) {
    ...
    // Construct the string directly from the byte array
    data = std::string(reinterpret_cast<char*>(dataBytes), n);
...
void DW1000Class::setBit(uint8_t data[], uint16_t n, uint16_t bit, bool val) {
	...
	shift = bit%8;
	if(val) {
		*targetByte |= (1UL << shift);// bitSet(*targetByte, shift);
	} else {
...
bool DW1000Class::getBit(uint8_t data[], uint16_t n, uint16_t bit) {
	...
	uint8_t targetByte = data[idx];
	shift = bit%8;
	bool bitValue = (targetByte & (1UL << shift)) != 0;
	return bitValue;// return bitRead(targetByte, shift); // TODO wrong type returned uint8_t instead of bool
	}
...
void DW1000Class::readBytes(uint8_t cmd, uint16_t offset, uint8_t data[], uint16_t n) {
    ...
    // Initialize the header based on whether offset is used
    if (offset == NO_SUB) {
        header[0] = READ | cmd;
    } else {
        header[0] = READ_SUB | cmd;
        if (offset < 128) {
            header[1] = static_cast<uint8_t>(offset);
            headerLen++;
        } else {
            header[1] = RW_SUB_EXT | static_cast<uint8_t>(offset);
            header[2] = static_cast<uint8_t>(offset >> 7);
            headerLen += 2;
        }
    }
    ...
    memset(&trans_desc, 0, sizeof(trans_desc)); // Zero out the transaction
    trans_desc.flags = SPI_TRANS_USE_RXDATA; // Use the RXDATA member of the union
    trans_desc.cmd = header[0]; // The first byte of the header is the command
    trans_desc.addr = (headerLen > 1) ? (header[1] | (header[2] << 8)) : 0; // The rest is the address
...
void DW1000Class::spiTransfer(uint8_t* header, uint16_t headerLen, uint8_t* data, uint16_t data_size) {
    // Create a transaction structure
    spi_transaction_t t;
    memset(&t, 0, sizeof(t)); // Zero out the transaction

    // Set up the transaction
    t.length = 8 * (headerLen + data_size); // Total length in bits
    t.tx_buffer = header; // Transmit buffer
    ...
    if (ret != ESP_OK) {
        if (ret == ESP_ERR_TIMEOUT) {
            ...
            ESP_LOGE("DW1000", "SPI transmission failed with error code: %d", ret);
        }
    }
}
...
void DW1000Class::writeBytes(uint8_t cmd, uint16_t offset, uint8_t data[], uint16_t data_size) {
	...
	uint16_t totalLength = headerLen + data_size;
	uint8_t* combinedData = new uint8_t[totalLength];

	memcpy(combinedData, header, headerLen);
	memcpy(combinedData + headerLen, data, data_size);

	spiTransfer(header, headerLen, combinedData, totalLength);
...
void DW1000Class::getPrettyBytes(uint8_t data[], char msgBuffer[], uint16_t n) {
    ...
    for(i = 0; i < n; i++) {
        ...
        b += snprintf(&msgBuffer[b], 6, "%d", (i+1)); // Allow up to 5 characters + null terminator
        msgBuffer[b++] = ':';
        msgBuffer[b++] = ' ';
        for(j = 0; j < 8; j++) {
...
void DW1000Class::getPrettyBytes(uint8_t cmd, uint16_t offset, char msgBuffer[], uint16_t n, int bufferSize) {
    uint16_t i, j, b;
    uint8_t* readBuf = (uint8_t*)malloc(n);
    readBytes(cmd, offset, readBuf, n);

    // Use a larger temporary buffer for snprintf to safely format each line
    char lineBuffer[50]; // Make sure this is large enough for one line
    b = sprintf(msgBuffer, "Reg: 0x%02x, bytes: %d\nB: 7 6 5 4 3 2 1 0\n", cmd, n);

    for(i = 0; i < n; i++) {
        uint8_t curByte = readBuf[i];
        int len = snprintf(lineBuffer, sizeof(lineBuffer), "%d: ", (i+1));
        for(j = 0; j < 8; j++) {
            len += snprintf(lineBuffer + len, sizeof(lineBuffer) - len, "%c ", ((curByte >> (7-j)) & 0x01) ? '1' : '0');
        }

        // Remove last space and add newline or null terminator
        lineBuffer[len - 1] = (i < n-1) ? '\n' : '\0';

        // Check if we have enough space left, then concatenate
        if ((b + len) < bufferSize) { // MSG_BUFFER_SIZE is the size of msgBuffer
            strcat(msgBuffer, lineBuffer);
            b += len;
        } else {
            // Handle the error - buffer overflow
            break;
        }
    }
