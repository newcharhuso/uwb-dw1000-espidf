...
void DW1000RangingClass::initCommunication(gpio_num_t myRST, gpio_num_t mySS, gpio_num_t myIRQ) {
	// reset line to the chip
	_RST              = myRST;
	_SS               = mySS;
...
void DW1000RangingClass::generalStart() {
	...
	_rangingCountPeriod = xTaskGetTickCount() * portTICK_PERIOD_MS;
}
...
void DW1000RangingClass::startAsAnchor(char address[], const uint8_t mode[], const bool randomShortAddress) {
    ...
    // Seed the random number generator - typically done once at the start
    srand(time(0));  
...
bool DW1000RangingClass::addNetworkDevices(DW1000Device* device, bool shortAddress) {
    ...
    for(uint8_t i = 0; i < _networkDevicesNumber; i++) {
        if((_networkDevices[i].isAddressEqual(device) && !shortAddress) || 
           (_networkDevices[i].isShortAddressEqual(device) && shortAddress)) {
    ...
    // Add the new device
    device->setRange(0);
    _networkDevices[_networkDevicesNumber] = *device; // Using copy assignment
    _networkDevices[_networkDevicesNumber].setIndex(_networkDevicesNumber);
	int16_t temp = _networkDevicesNumber;
	temp ++;
	_networkDevicesNumber = temp;
    return true;
}
...
bool DW1000RangingClass::addNetworkDevices(DW1000Device* device) {
    ...
    // Add the new device
    if(_type == ANCHOR) {
        // For now let's start with 1 TAG
...
void DW1000RangingClass::removeNetworkDevices(int16_t index) {
    ...
    if (_networkDevicesNumber == 1) {
		...
		temp = _networkDevicesNumber;
		temp --;
        _networkDevicesNumber = temp;
    }
    else {
        // Translate all elements after the one we want to delete
        for (int16_t i = index; i < _networkDevicesNumber - 1; i++) {
            _networkDevices[i] = _networkDevices[i + 1]; // Using copy assignment
